...
- В отличие от [[POST]], PUT обычно **идемпотентен** (несколько одинаковых запросов дают один результат).
...
- Отличие от [[POST]]: PUT обычно **обновляет конкретный ресурс**, POST — создаёт новый.
