`forEach` — это метод протокола [[Sequence 1]], который позволяет выполнить указанное действие (замыкание) **для каждого элемента** коллекции или последовательности. Это самый простой и декларативный способ выразить «сделай что-то с каждым элементом, но не создавай новый результат».
