...
- **Value Types**: При присваивании или передаче в функцию создается **независимая копия** данных.
...
| **Наследование**                 | Нет                                     | Да                           |
| **[[Deinit]] / [[ARC]]**         | Не нужен                                | Есть (счетчик ссылок)        |
...
graph TD
    subgraph Stack["📚 Стек (Stack) - быстрый доступ, LIFO"]
        direction TB
        S1["Value Types:<br/>• Int x = 10<br/>• Bool flag = true<br/>• Double pi = 3.14"]
...
func processReference() {
    ...
    // ... объект живет, пока есть ссылки
}
```

### Итог

**Value Types ([[struct]], [[enum]]) — выбирайте по умолчанию:**
- Независимые копии при присваивании
- Безопасность в многопоточной среде
- Хранение на стеке (быстро)
- Нет проблем с [[retain cycle]]s

**Reference Types ([[class]]) — выбирайте когда нужно:**
- Общая идентичность (несколько ссылок на один объект)
- Наследование
- deinit для очистки ресурсов
- Наблюдение за жизненным циклом

Понимание различий между [[Value Type|value]] и [[reference type]]s — фундамент для написания эффективного, безопасного и предсказуемого кода на Swift .