Вот подробная статья для Obsidian по термину **Copy-On-Shadow (COS)** в контексте Swift и iOS-разработки. Текст содержит определения, принципы работы, сравнение с Copy-On-Write (COW), примеры реализации, анализ производительности, сценарии применения и теги.
...
| **Встроенная поддержка** | Да (Array, Dictionary, Set, String) | Нет (кастомная реализация) |
...
- **COW** — стандарт Swift для `Array`, `Dictionary`, `Set`, `String`. Он уже очень хорошо оптимизирован .
