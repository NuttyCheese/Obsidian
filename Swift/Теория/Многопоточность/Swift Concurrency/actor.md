...
| Использовать actor для UI-логики                   | Неэффективно (не на [[main\|main thread]]) | Для UI — `@MainActor`                                                                 |
...
| `class` + [[NSLock\|lock]]     | Ручная               | Нет          | Сложно         | Высокая   | Только если нет альтернативы |
