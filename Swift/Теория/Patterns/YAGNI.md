Вот **полное, подробное и максимально актуальное** (на 2026 год) руководство по принципу **YAGNI** («You Aren’t Gonna Need It» — «Тебе это не понадобится») в контексте разработки на Swift.
...
```mermaid
flowchart TD
    OverEngineering["Переинжиниринг<br>→ 800 строк, 12 абстракций, 8 generic"] --> Bad["Нарушение YAGNI<br>→ сложно читать, тестировать, поддерживать"]

    Minimal["Минимальный код<br>→ только то, что нужно сейчас"] --> Good["Соблюдение YAGNI<br>→ быстро писать, легко менять"]

    Good --> "Быстрее релиз"
    Good --> "Меньше багов"
    Good --> "Легче рефакторинг"
