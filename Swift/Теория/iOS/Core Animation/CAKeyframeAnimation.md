Вот подробная статья для Obsidian по термину **CAKeyframeAnimation** в контексте iOS-разработки на Swift/UIKit. Текст содержит определения, архитектуру, детальное описание свойств, сравнение с CABasicAnimation, примеры кода от простого к сложному (анимация по пути, по значениям, по времени), best practices и теги.
