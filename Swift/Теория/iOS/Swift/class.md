...
| ARC overhead                    | Каждый объект — отдельная аллокация + retain/release | Копируется только при изменении (COW)          | Много маленьких объектов — лучше struct |
...
| KVO, NotificationCenter, delegates            | Требуется стабильная ссылка (identity)                 | `observe(\.value)`, `NotificationCenter` observers |
...
Удачи с правильным выбором между class и struct в твоём коде! 🏛️