**`AnyObject`** — это специальный протокол в [[Swift]], который ограничивает тип **только классами** ([[reference type]]s).  
...
| [[Dynamic Dispatch]]?              | Да (если протокол)                                                    | Да (всегда, классы используют [[Table Dispatch\|vtable]])               | `AnyObject` чуть быстрее при вызове методов        |
...
| [[KVO]], notifications, [[delegate]]s (старый код) | Многие старые API используют `AnyObject`          | `observe(\.value) { (obj: AnyObject?, change) in ... }` |
