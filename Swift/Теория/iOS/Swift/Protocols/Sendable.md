...
| ----------------------------------------- | -------------------------------------------------------------- | ----------------------------- |
| Все [[Value Type]] без мутабельных ссылок | struct, enum, tuple                                            | Если все свойства Sendable    |
| Основные типы [[Foundation]]              | [[String]], [[Data]], [[Date]], [[UUID]], [[URL]], [[Decimal]] | Полностью Sendable            |
| Коллекции                                 | [[Array]]<T>, [[Dictionary]]<Key, Value>, [[Set Collection]]<T>           | Если T / Key / Value Sendable |
...
| --------------------------------- | --------------------------------------------- | --------------------------------------------------------------------- |
| class (обычный)                   | [[reference semantic]] + mutable shared state | `final class` + `Sendable` (если immutable) или `@unchecked Sendable` |
...
- **any Protocol** — избегай в Task / actor-передачах → предпочитай [[some Protocol]] или [[generic]]
