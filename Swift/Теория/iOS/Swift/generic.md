...
### 3. Ограничения типов (Constraints) — ключ к мощности
...
| `T: AnyObject`               | Только классы (reference types)            | `weak var delegate: T?` |
| `T: Sendable`                | Безопасность в concurrent коде (Swift 6+)  | Акторы, Task, async let |
...
### 4. Associated Types в протоколах (самый мощный паттерн)
...
- Не бойся generics — они **не замедляют** код в релизе (-O), а часто ускоряют  
...
Удачи с универсальным, читаемым и быстрым кодом! 🧬