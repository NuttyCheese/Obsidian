...
- **AVL-дерево**, **красно-чёрное дерево** (используются внутри [[Dictionary]] / [[Set Collection]])
- **Дерево решений**, **четырёхугольное дерево** (Quadtree), **октодерево** (Octree) — в играх и AR
- **DOM-дерево** (в WebKit), **иерархия [[UIView]]**, **файловая система**
...
### Пример 3 — обход дерева UI-компонентов ([[UIView]] hierarchy)
