...
|-----------------------------------------------------|----------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|-----------------------|
| `scene(_:willConnectTo:options:)`                   | Когда сцена создаётся или восстанавливается (самый важный!)                     | Установка rootViewController, загрузка состояния из userActivity                                 | Установка начального экрана |
| `sceneDidBecomeActive(_:)`                          | Сцена стала активной (на экране)                                                 | Запуск анимаций, обновление UI, возобновление сетевых запросов                                  | Начало работы сцены |
| `sceneWillResignActive(_:)`                         | Сцена теряет активность (например, переключение на другое окно)                 | Сохранение состояния, пауза анимаций, остановка таймеров                                        | Подготовка к уходу в фон |
| `sceneDidEnterBackground(_:)`                       | Сцена ушла в фон                                                             | Сохранение данных, отключение ненужных обновлений                                               | Экономия батареи |
| `sceneWillEnterForeground(_:)`                      | Сцена возвращается на экран                                                      | Обновление данных, перезапуск анимаций                                                           | Возобновление работы |
| `sceneDidDisconnect(_:)`                            | Сцена уничтожается (окно закрыто)                                                | Очистка ресурсов, отмена подписок                                                               | Завершение сцены |
| `scene(_:openURLContexts:)`                         | Открытие по URL (universal links, custom schemes)                                | Обработка deep link                                                                              | Deep linking |
...
| `scene(_:continue:)`                                | Восстановление из userActivity                                                   | Восстановление состояния из NSUserActivity                                                       | Handoff, universal links |
...
Удачи с многоконным и адаптивным приложением в твоём проекте! 🪟