Вот **полное, подробное и максимально актуальное** (на февраль 2026 года) руководство по **сериальным очередям** (`DispatchQueue` без атрибута `.concurrent`) в Swift и Grand Central Dispatch (GCD).

### 1. Что такое сериальная очередь в GCD
