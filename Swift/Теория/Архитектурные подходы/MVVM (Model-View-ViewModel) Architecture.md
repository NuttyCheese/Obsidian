...
3.  **Замыкания ([[closure]]) / Делегаты ([[Delegate]]):** Более простой, ручной способ. ViewModel может иметь свойство-замыкание `var onUpdate: (() -> Void)?`, которое View вызывает для подписки, а ViewModel вызывает при изменениях.
