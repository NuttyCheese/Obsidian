...
Это один из самых важных и одновременно самых спорных механизмов в языке, особенно в контексте **Swift 6 strict concurrency** и **value types** (struct, enum).
...
- **Протоколы** — делай методы `mutating` только если это часть семантики (например, `Collection` имеет `mutating func append`)  
...
Удачи с правильной мутабельностью и value semantics в Swift! 🔄