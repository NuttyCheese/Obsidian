...
```mermaid
graph LR
    A[Дизайнер в Sketch for Mac] --> B[Создает макеты с использованием Символов и Стилей];
    B --> C[Загружает документ в Sketch Workspace (облако)];

    subgraph Разработчик iOS
        D[Открывает веб-приложение Sketch / браузер];
        E[Инспектирует слой: размеры, отступы, цвета];
        F[Экспортирует иконки и ресурсы];
        G[Тестирует прототип на iPhone через Sketch – View and Mirror];
    end
    ...
    E --> H[Код Swift/UIKit: NSLayoutConstraint, UIColor, UIFont];
    F --> I[Xcode Assets.xcassets];
    G --> J[Проверка логики переходов и поведения на реальном устройстве];
    
    H --> K[Готовое приложение];
    I --> K;
